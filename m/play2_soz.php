...
if(getisset("finish")) {
	...
	dGuncelle("play",
	array(
		"u2info" => json_encode(oturum("userinfo")),
		"u2score" => $score
		),
...
if(getisset("first")) {
	...
	$kim = ksorgu("play","WHERE id=$id AND u2={$_SESSION['uid']}");
	...
	if($kim!=0) {
		...
		if($play['u2score']!="") {
			...
			if(count($u1info)<7) {
				...
				oturum("playgame",get("first"));
				yonlendir("play2_soz.php"); // oyun ekranına yönlendirelim.	
			}
		}
	} else {
		yonlendir("profile.php");
...
<puan style="right:10px"></puan>
</div>
<?php 
oturumAc();
//print_r($_SESSION);
if(!oturumisset("playgame")) {ob_start(); yonlendir("profile.php");}
$game = oturum("playgame");
$sorular = kd(ksorgu("play","WHERE id=$game"));
$sorular2 = json_decode($sorular['sorular']);
$vs = user($sorular['u1']);
//$kat = kd(content($sorular['kat']));
 ?>
...
<score id="u2score">0</score>
 ...
 <div class="sorular" style="    max-width: 700px;
    margin: 0 auto;">
 <?php foreach($sorular2 AS $s) {  
 $soru = kd(ksorgu($sorular['kat'],"WHERE id = $s"));
	
 ?>
 
 <div class="soru">
<h1><?php e($soru['anlam']); ?></h1>
 <div class="cevaplar">
...
$diger = ksorgu($sorular['kat'],"ORDER BY rand() LIMIT 0,3");
$k = 1;
while($c = kd($diger)) { 
	$cevaps[$k]['id'] = $c['id'];
...
<a href="result.php?id=<?php e(oturum("playgame")); ?>" class="ui-btn wisteria cevap"><i class="fa fa-area-chart"></i> <?php le("Oyun Detayları") ?></a>
