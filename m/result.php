...
<?php
$id = veri(get("id"));
$play = ksorgu("play","WHERE (u1 = {$user['id']} OR u2 = {$user['id']}) AND id=$id AND u1info IS NOT NULL AND u2info IS NOT NULL");
...
if($p['u1score']==$p['u2score']) {
	$durum = "Berabere";
	$renk = "sunflower";
	$durum2="Berabere kaldığın bu oyun hakkında bir şeyler yaz";
} elseif($p['u1score']>$p['u2score']) {
	if($user1=="Sen") {
		$durum = "Kazandın!";
		$renk = "nephritis";
		$durum2="Kazandığın bu oyun hakkında bir şeyler yaz";
	} else {
		$durum = "Kaybettin!";
		$renk = "pomegranate";
		$durum2="Kaybettiğin bu oyun hakkında bir şeyler yaz";
	}
} elseif($p['u1score']<$p['u2score']) {
	if($user2=="Sen") {
		$durum = "Kazandın!";
...
<div class="panelAlan">
<?php 
$kat = kd(content($p['kat']));
if($kat['alt']=="") { //sözcük geliştirme değilse ?>
...
<?php if($p['kat']=="ing_tur") { ?>
<h1><?php le("Şimdilik Oyun Detayları Gösterilmiyor"); ?></h1>
<?php } else { ?>
<div class="soru">
