...
$hiyerarsi2="";
function hiyerarsi2($id) {
global $hiyerarsi2;
	$kat = kd(ksorgu("urun_kategori","WHERE id=$id"));
	$isim = $kat['isim'];
	$a = $kat['id'];
		$hiyerarsi2 =  "<a href='urunler.php?id=$a'>$isim</a> <i class='fa fa-caret-right'></i> " . $hiyerarsi2;
	if($kat['ust']!="") {
		hiyerarsi2($kat['ust']);
	} else {
		$hiyerarsi2 = "<a href='urunler.php'>Ürünler</a> <i class='fa fa-caret-right'></i> " . $hiyerarsi2 . "</div>";
	}
	return $hiyerarsi2;
}
...
?>
<?php function dEkle($tablo, $bilgiler,$debug=false) {
