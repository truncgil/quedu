...
*/
@$html_istisna = array("mesaj","icerik");
header('Content-Type: text/html; charset=utf-8');
setlocale(LC_ALL, 'tr_TR.UTF8');

if(!isset($sunucu)) {
	echo "\$sunucu isminde bir sunucu IP/Host değişkeni tanımlayınız <br />";
	if(!isset($kullanici)) {
		echo "\$kullanici isminde sunucuya bağlanılacak kullanıcı adı değişkeni tanımlayınız <br />";
		if(!isset($parola)) {
			echo "\$parola isminde sunucuya bağlanılacak bir parola değişkeni tanımlayınız <br />";
		} else {
			if(mysql_pconnect($sunucu, $kullanici, $parola)) {
			$paralel_bag = mysql_pconnect($sunucu, $kullanici, $parola); 
			
			} else {
			echo "Sunucu bağlantı hatası";
			}		
		}
	}
}
$paralel_bag = @mysql_pconnect($sunucu, $kullanici, $parola); 
mysql_set_charset('utf8',$paralel_bag);
mysql_query("SET NAMES utf8");
mysql_query("SET SESSION character_set_results = 'UTF8'");
mysql_query("set SESSION character_set_client = 'UTF8'");
mysql_query("set SESSION character_set_connection = 'UTF8'");
if(!isset($veritabani)) {
	echo "\$veritabani isminde sunucudaki erişilecek olan veritabanı değişkenini giriniz. <br />";
}
...
function js_yonlendir($url) {
	echo "<script>location.href='$url';</script>";
}
function error($level=E_USER_NOTICE) {
    $caller = @next(debug_backtrace());
    $hata = mysql_error();
	@trigger_error($hata, $level);
	dEkle("error",array(
		"e" =>  $hata,
		"tarih" => simdi(),
		"ip" => $_SERVER['REMOTE_ADDR'],
		"detay" => json_encode($caller),
		"file" => $caller['file']
	));
}
function postGuncelle($tablo,$alan,$deger){
	global $_POST;
	dGuncelle($tablo,$_POST,"$alan=$deger");
}
function alansorgu($tablo,$alan,$deger){
	return ksorgu($tablo,"WHERE $alan = $deger");
}
function alanSil($tablo,$alan,$deger){
	sil($tablo,"$alan=$deger");
}
...
function dekle($tablo, $bilgiler) {
	...
	foreach ($bilgiler as $anahtar => $deger) { // her anahtara ait değerleri alalım
		if (is_string($deger)) {
			if(!in_array($anahtar, $html_istisna)) {
...
//	e($ekleSQL);
	//return $ekleSQL;
    mysql_select_db($veritabani, $paralel_bag);
	//echo $ekleSQL;
    $sonuc = mysql_query($ekleSQL, $paralel_bag) or error();
...
$sorgu = mysql_query($sorgu, $paralel_bag) or error();
...
function sil($tablo,$kriter) {
  ...
  mysql_select_db($veritabani, $paralel_bag);
  $sonuc = mysql_query($silSQL, $paralel_bag) or error();
  ...
  $guncelleSQL = sprintf("UPDATE %s SET %s WHERE %s",
					 $tablo,
					  $set,
					   $where);
  mysql_select_db($veritabani, $paralel_bag);
  $Sonuc = mysql_query($guncelleSQL, $paralel_bag) or error();
}
function dGuncelle($tablo,$set,$where) {
	global $veritabani;
	global $paralel_bag;
	global $html_istisna;
	$toplamSet = "";
...
function kSorgu($tablo,$diger="",$baslangic=0,$bitis=0) {
	...
	if ($bitis!=0) {
		...
		$sorgu = mysql_query($sorgu . $limit, $paralel_bag) or error();
		$totalRows_sorgu = @mysql_num_rows($sorgu);
		if ($totalRows_sorgu==0) {
			return 0;
		} else {
...
?>
<?php function rsifre($hane=6){
	$kripto = kripto(rand(0,99999) . rand(0,99999));
	if($hane>32) { //32 haneden fazla şifreleme yapamaz
		$hane = 32;
	}
	return substr($kripto,0,$hane);
} ?>
<?php function brsil($metin){
	return str_replace("<br />","",$metin);
} ?>
<?php function st($metin){
...
function slug($string, $ayrac='.') {
   ...
   $table = array(
            ...
            'à'=>'a', 'á'=>'a', 'â'=>'a', 'ã'=>'a', 'ä'=>'a', 'å'=>'a', 'æ'=>'a', 'ç'=>'c', 'è'=>'e', 'é'=>'e',
            'ê'=>'e', 'ë'=>'e', 'ì'=>'i', 'ı'=>'i', 'í'=>'i', 'î'=>'i', 'ï'=>'i',  'ğ'=>'g', 'Ğ'=>'g', 
			'Ö'=>'o', 'Ü'=>'u', 'İ'=>'i', 'Ş'=>'s', 'ş'=>'s', 'ð'=>'o', 'ñ'=>'n', 'ò'=>'o', 'ó'=>'o',
            'ô'=>'o', 'õ'=>'o', 'ö'=>'o', 'ø'=>'o', 'ù'=>'u', 'ú'=>'u','ü'=>'u', 'û'=>'u', 'ý'=>'y', 'ý'=>'y', 'þ'=>'b',
            'ÿ'=>'y', 'Ŕ'=>'R', 'ŕ'=>'r', '/' => $ayrac, ' ' => $ayrac, ',' => '', '&' => '', "'" => '', "+" => '_', ":" => ''
    );
...
?>
<?php function kripto($veri,$diger = "") {
	$veri = md5($diger. $veri . $diger);
	return $veri;
}
?>
...
?>
<?php function posta($kim, $baslik, $mesaj) {
...
function tarihf($tarih,$format="Y") {
		$tarihe = str_replace("/","-",$tarih);
		$tarihe = strtotime($tarihe);
	$tarihs = date($format, $tarihe);
//	$tarih = date_format(date_create($tarih), $format);
	return $tarihs;
}
?>
...
function para($deger,$sembol='TL',$r=2)
{
$veri =  number_format($deger,$r,",",".");
if($deger!=0) { 
    return $veri . " " . $sembol;
}
}
