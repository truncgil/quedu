...
if(getisset("first")) {
	...
	if($kim!=0) {
		...
		if(!oturumisset("selectuser")) {
			print_r($user);
			$u = kd(ksorgu("uyeler","where (seviye<>'Yonetici' AND id<>{$user['id']} AND bolum='{$user['bolum']}') ORDER BY rand() LIMIT 1"));
			oturum("selectuser",$u['id']);
...
<a href="selectuser.php?rasgele" class="ui-btn alizarin cevap" ><i class="fa fa-repeat"></i> <?php le("Başkasıyla Tekrar Oyna") ?></a>
